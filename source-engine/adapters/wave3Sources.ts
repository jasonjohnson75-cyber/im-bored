...
const source = (sourceId: string, urls: string[], confidence = 0.9, extractor = extractJsonLdEvents) =>
  createStructuredHtmlAdapter({ sourceId, listingUrls: () => urls, extractor, confidence, filter: regional });

export const southBendVpaAdapter = source('south-bend-vpa', ['https://sbvpa.org/things-to-do/'], 0.93);
export const debartoloAdapter = source('debartolo-performing-arts-center', ['https://performingarts.nd.edu/events/'], 0.94, extractDebartoloEvents);
...
export const elkhartCountyParksAdapter = source('elkhart-county-parks', ['https://elkhartcountyparks.org/events/'], 0.94, extractElkhartCountyParksEvents);
